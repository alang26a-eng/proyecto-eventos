...
import { findIdentity } from '../repositories/users.repository.js';
import { hashPassword, comparePassword } from '../utils/hash.js';
import { verifyToken, signToken } from '../utils/jwt.js';
import { userDTO, currentUserDTO } from '../dto/user.dto.js';
import HttpError from '../utils/HttpError.js';

export const createSession = user => ({ token: signToken({ ...user, _id: user.id }), user: userDTO(user) });
export const describeCurrentUser = user => currentUserDTO(user);
...
export function currentIdentity(token) {
  try {
...
export async function accessIdentity(token) {
